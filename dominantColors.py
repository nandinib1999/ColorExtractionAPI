...
def plotColorClusters(img, clusters):
    cluster_map, hex_colors, label_counts, kmeans = TrainKMeans(img, clusters)
...
def start(IMG_PATH, clusters):
    master_list = []
    img = Image.open(IMG_PATH)
    df = plotColorClusters(img, clusters)
    for ind, row in df.iterrows():
        color_dict = {}
        color_dict['id'] = row['color']
        color_dict['name'] = row['color_name']
        color_dict['percentage'] = row['Percentage']
        master_list.append(color_dict)
